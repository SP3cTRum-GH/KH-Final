import axios from "axios";
import { API_SERVER_HOST } from "./HostUrl";
import jwtAxios from "../util/jwtUtil";

const prefix = `${API_SERVER_HOST}/api/review`;

// 리뷰 등록
export const postReview = async (product) => {
  const res = await jwtAxios.post(`${prefix}/user`, product);
  return res.data;
};

// 리뷰 1개 가져오기
export const getReviewOne = async (reviewNo) => {
  const res = await axios.get(`${prefix}/public/${reviewNo}`);
  return res.data;
};

// 유저 리뷰 리스트
export const getUserReviewList = async (memberId) => {
  const res = await jwtAxios.get(`${prefix}/user?memberId=${memberId}`);
  return res.data;
};

// 페이징 리스트
export const getReviewList = async (pageParam, productNo) => {
  const { page, size } = pageParam;
  const res = await axios.get(`${prefix}/public/list?productNo=${productNo}`, {
    params: { page, size },
  });

  return res.data;
};

// 리뷰 수정
export const updateReview = async (reviewNo, data) => {
  const res = await jwtAxios.put(`${prefix}/user/${reviewNo}`, data);
  return res.data;
};

// 리뷰 삭제
export const deleteReview = async (reviewNo) => {
  const res = await jwtAxios.post(`${prefix}/admin/${reviewNo}`);
  return res.data;
};

// 리뷰 수량
export const reviewCount = async (productNo) => {
  const { data } = await axios.get(
    `${prefix}/public/count?productNo=${productNo}`
  );
  // API returns either a number or an object like { count: number }
  return typeof data === "number" ? data : data?.count ?? 0;
};
